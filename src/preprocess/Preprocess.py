...
class Preprocess:
    ...
    def _getXmlTree(self, xmlContent):
        return etree.fromstring(xmlContent)


    def _docx2txt(self, docx_string):
        WORD_NAMESPACE = '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}'
        PARA = WORD_NAMESPACE + 'p'
        TEXT = WORD_NAMESPACE + 't'

        tree = self._getXmlTree(docx_string)
    ...
    def _parseDocx(self, docx_string):
        WORD_NAMESPACE = '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}'
        R = WORD_NAMESPACE + 'r'
    ...
    def _get_polar_files(self, sentence, pos, neg):
        pos_features = []
        neg_features = []
        count_pos = 0
        count_neg = 0
    ...
    def process_train_samples(self, path):
        # poses = {}
        # pos_count = {}
        s_array = []
        p_array = []
        ...
        for parent, dirnames, filenames in os.walk(path):
            for filename in filenames:
                # print filename
                sentences = _parseDocx(_getXml(path + filename))
        ...
        _get_polar_files(s_array, p_array, n_array)
    ...
    def _process_segment(self, texts):
        segs = segment(texts)
        return [term for term in segs]


    def _process_segment_by_sentences(self, texts):
        sentences = self.cut_sentence(texts)
        result = []
        for sentent in sentences:
            segs = segment(sentent)
            result.append([term for term in segs])

        return result

    def segment(self, filename):
        texts = self._docx2txt(self._getXml(filename))
        return [self._process_segment(texts), self._process_segment_by_sentences(texts)]
